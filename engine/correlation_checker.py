...
from data_loader.config import COMMITTED_ALPHAS_PATH


class CorrelationChecker:
    """日收益相关性分析与拦截器"""

    def __init__(self, db_path: Path = COMMITTED_ALPHAS_PATH):
    ...
    def commit_alpha(self, alpha_id: str, dates: List[Any], daily_pnl: np.ndarray) -> bool:
        """将新因子加入已提交因子库"""
        new_df = pl.DataFrame({
            "date": dates,
            alpha_id: daily_pnl
        ...
        print(f"[CorrChecker] 成功将因子 '{alpha_id}' 入库，当前库内因子数: {len(self.alphas_df.columns) - 1}")
        return True
